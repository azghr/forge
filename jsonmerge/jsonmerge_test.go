...
func TestMerge(t *testing.T) {
	...
	t.Run("nested merge", func(t *testing.T) {
		a := map[string]any{"a": 1, "nested": map[string]any{"x": 10}}
		b := map[string]any{"nested": map[string]any{"x": 20}, "c": 3}
		jsonmerge.Merge(a, b)
		if a["c"] != 3 {
			t.Errorf("expected c=3, got %v", a["c"])
		}
		n := a["nested"].(map[string]any)
		if n["x"] != 20 {
	...
	t.Run("deep nested", func(t *testing.T) {
		a := map[string]any{
			"level1": map[string]any{
				"level2": map[string]any{"v": 1},
			},
		}
		b := map[string]any{
			"level1": map[string]any{
				"level2": map[string]any{"v": 2, "w": 3},
			},
		}
		jsonmerge.Merge(a, b)
		l1 := a["level1"].(map[string]any)
	...
	t.Run("non-map override", func(t *testing.T) {
		a := map[string]any{"k": map[string]any{"v": 1}}
		b := map[string]any{"k": 42}
		jsonmerge.Merge(a, b)
		if a["k"] != 42 {
	...
	t.Run("slice default replace", func(t *testing.T) {
		a := map[string]any{"s": []any{1, 2}}
		b := map[string]any{"s": []any{3, 4}}
		jsonmerge.Merge(a, b)
		s := a["s"].([]any)
...
func TestDiff(t *testing.T) {
	...
	t.Run("nested diff", func(t *testing.T) {
		a := map[string]any{"y": map[string]any{"v": 2, "w": 3}}
	...
	t.Run("empty maps", func(t *testing.T) {
		got := jsonmerge.Diff(map[string]any{}, map[string]any{})
		if len(got) != 0 {
			t.Errorf("expected no diffs, got %v", got)
		}
	})
	...
	t.Run("slices differ", func(t *testing.T) {
		a := map[string]any{"s": []any{1, 2, 3}}
		b := map[string]any{"s": []any{1, 2, 4}}
		got := jsonmerge.Diff(a, b)
...
func TestConcurrentSafety(t *testing.T) {
	...
	for range 30 {
		go func() {
			<-run
			a := map[string]any{"x": 1, "nested": map[string]any{"v": 2}}
			b := map[string]any{"nested": map[string]any{"v": 3}, "z": 4}
			jsonmerge.Merge(a, b)
			jsonmerge.Merge(a, b, jsonmerge.WithSliceMode(jsonmerge.SliceAppend))
			jsonmerge.Diff(a, b)
			done <- struct{}{}
